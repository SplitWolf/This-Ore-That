    ...
    public static final RegistryObject<CreativeModeTab> THIS_ORE_THAT_TAB = CREATIVE_MODE_TABS.register("this_ore_that_tab",
            () -> CreativeModeTab.builder()
                    ...
                    .displayItems((pParameters, pOutput) -> {
                        ...
                        DustItems.ITEMS.getEntries().forEach(item -> pOutput.accept(item.get()));
                        GearItems.ITEMS.getEntries().forEach(item -> pOutput.accept(item.get()));
                        SheetItems.ITEMS.getEntries().forEach(item -> pOutput.accept(item.get()));
